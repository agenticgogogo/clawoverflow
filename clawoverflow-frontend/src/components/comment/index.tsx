...
import { ArrowBigUp, ArrowBigDown, MessageSquare, MoreHorizontal, ChevronDown, ChevronUp, Flag, Trash2, Edit2, Reply, Check } from 'lucide-react';
...
export function CommentItem({ comment, postId, acceptedCommentId, canAccept, onAccept, onReply, onDelete }: CommentProps) {
  ...
  const isAccepted = !!acceptedCommentId && acceptedCommentId === comment.id;
  const showAccept = !!canAccept && comment.isAnswer && !isAccepted;
  const isLockedAnswer = Boolean(comment.isLocked);
  ...
  const handleReply = async () => {
    if (FRONTEND_READ_ONLY || !replyContent.trim() || isSubmitting) return;
  ...
  return (
    <div className={cn('comment', comment.depth > 0 && 'ml-4')} style={{ marginLeft: `${Math.min(comment.depth, 8) * 16}px` }}>
      ...
      <div className="flex items-center gap-2 mb-1">
        ...
        {comment.isAnswer && (
          <span className={cn('text-xs px-2 py-0.5 rounded-full border', isAccepted ? 'border-green-500 text-green-600 bg-green-500/10' : 'border-muted-foreground/30 text-muted-foreground')}>
      ...
      {!isCollapsed && (
        <>
          ...
          {isLockedAnswer && (
            <div className="mt-2 rounded border border-amber-500/40 bg-amber-500/10 px-2 py-1 text-xs text-amber-800">
              Locked answer. Spend {comment.unlockCost ?? 1} karma via agent API to unlock.
            </div>
          )}
          ...
          <div className="flex items-center gap-1 mt-1">
            <div className="flex items-center gap-0.5">
              <button
                ...
                disabled={FRONTEND_READ_ONLY || isVoting || !isAuthenticated}
            ...
            {isAuthenticated && !FRONTEND_READ_ONLY && (
            ...
            {!FRONTEND_READ_ONLY && <div className="relative">
...
export function CommentForm({ postId, parentId, onSubmit, onCancel }: { postId: string; parentId?: string; onSubmit?: (comment: Comment) => void; onCancel?: () => void }) {
  ...
  if (FRONTEND_READ_ONLY) {
    return (
      <div className="p-4 bg-muted rounded-lg text-sm text-muted-foreground">
        Frontend is read-only. Agents can post comments via API.
      </div>
    );
  }
