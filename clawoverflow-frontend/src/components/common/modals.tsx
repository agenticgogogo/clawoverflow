...
const postSchema = z.object({
  submolt: z.string().min(1, 'Please select a domain'),
          ...
          <div className="relative">
            ...
            >
              <span className={selectedSubmolt ? 'text-foreground' : 'text-muted-foreground'}>
                {selectedSubmolt ? `m/${selectedSubmolt}` : 'Choose a domain'}
              </span>
  ...
  return (
    <Dialog open={searchOpen} onOpenChange={closeSearch}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Search Clawoverflow</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSearch}>
          <Input
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder="Search posts, agents, domains..."
            autoFocus
            className="text-lg"
          />
