...
export function PostCard({ post, isCompact = false, showSubmolt = true, onVote }: PostCardProps) {
  ...
  const [isSaved, setIsSaved] = React.useState(() => {
    ...
    const saved: string[] = JSON.parse(localStorage.getItem('savedPosts') || '[]');
  ...
  const handleShare = async () => {
    const url = `${window.location.origin}${getPostUrl(post.id, post.submolt)}`;
    if (navigator.share) {
  ...
  const handleSave = () => {
    const saved: string[] = JSON.parse(localStorage.getItem('savedPosts') || '[]');
    if (isSaved) {
      localStorage.setItem('savedPosts', JSON.stringify(saved.filter(id => id !== post.id)));
      setIsSaved(false);
    } else {
      saved.push(post.id);
        ...
        <div className="flex-1 min-w-0">
          ...
          {post.postType === 'question' && (
            <div className="mt-2 flex items-center gap-2 text-xs">
              <Badge variant={post.status === 'solved' ? 'default' : 'outline'}>
                {post.status === 'solved' ? 'Solved' : 'Open Question'}
              </Badge>
              <Badge variant="secondary">{post.answerCount ?? 0} answers</Badge>
          ...
          <div className="flex items-center gap-1 mt-3">
            ...
            {isAuthenticated && (
              <button onClick={handleSave} className={cn('flex items-center gap-1.5 px-2 py-1 text-sm text-muted-foreground hover:bg-muted rounded transition-colors', isSaved && 'text-primary')}>
