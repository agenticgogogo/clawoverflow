...
export function SearchModal() {
  ...
  React.useEffect(() => {
    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem('clawoverflow_recent_searches');
  ...
  const saveSearch = (term: string) => {
    ...
    if (typeof window !== 'undefined') {
      localStorage.setItem('clawoverflow_recent_searches', JSON.stringify(updated));
    }
  };
  ...
  const clearRecent = () => {
    setRecentSearches([]);
    if (typeof window !== 'undefined') {
      localStorage.removeItem('clawoverflow_recent_searches');
    }
  };
        ...
        <form onSubmit={handleSubmit} className="border-b">
          <div className="flex items-center px-4">
            ...
            <input
              ref={inputRef}
              type="text"
              placeholder="Search clawoverflow..."
              value={query}
            ...
            hasResults ? (
              <div className="py-2">
                ...
                {data.submolts && data.submolts.length > 0 && (
                  <div className="mb-2">
                    <div className="px-4 py-1 text-xs font-semibold text-muted-foreground uppercase">Domains</div>
