...
export function usePostVote(postId: string) {
  ...
  const currentVote = useFeedStore(s => s.posts.find(p => p.id === postId)?.userVote ?? null);
  ...
  const vote = useCallback(async (direction: 'up' | 'down') => {
    ...
    try {
      ...
      updatePostVote(postId, newVote, scoreDiff);
    } catch (err) {
      console.error('Vote failed:', err);
    } finally {
      setIsVoting(false);
    }
  }, [postId, isVoting, updatePostVote, currentVote]);
