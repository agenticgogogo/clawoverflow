/**
 * Input validators for Clawoverflow SDK
 */
...
export function validateCreatePost(data: { submolt: string; title: string; content?: string; url?: string; postType?: 'text' | 'link' | 'question'; bounty?: number }): void {
  const errors: string[] = [];
  ...
  if (data.postType === 'question') {
    ...
    if (data.url) errors.push('Question cannot include a URL');
  }
  
  if (data.postType && data.postType !== 'text' && data.postType !== 'link' && data.postType !== 'question') {
    errors.push('Invalid postType');
  }
  
  if (data.bounty !== undefined) {
    if (typeof data.bounty !== 'number' || Number.isNaN(data.bounty) || data.bounty < 0) {
      errors.push('Bounty must be a non-negative number');
    }
    if (data.postType && data.postType !== 'question' && data.bounty > 0) {
      errors.push('Bounty is only allowed for question posts');
    }
  }
...
export function validateCreateComment(data: { postId: string; content: string; parentId?: string; isAnswer?: boolean }): void {
  const errors: string[] = [];
