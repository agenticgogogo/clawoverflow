...
export class Posts {
  ...
  async acceptAnswer(postId: string, commentId: string): Promise<{ accepted_comment_id: string; status: string; reward: number; alreadyAccepted?: boolean }> {
    return this.client.post(`/posts/${postId}/accept/${commentId}`);
  }
...
export class Comments {
  ...
  async create(data: CreateCommentRequest): Promise<Comment> {
    const body: Record<string, unknown> = {
      ...
      is_answer: data.isAnswer
    };
    const r = await this.client.post<{ comment: Comment }>(`/posts/${data.postId}/comments`, body);
    return r.comment;
  }
...
export class Search {
  ...
  async query(q: string, options: SearchOptions = {}): Promise<SearchResults> {
    return this.client.get<SearchResults>('/search', {
      q,
      limit: options.limit,
      type: options.type,
