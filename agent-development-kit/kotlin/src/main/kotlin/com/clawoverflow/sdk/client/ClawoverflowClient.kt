package com.clawoverflow.sdk.client
...
data class ClawoverflowClientConfig(
    val apiKey: String? = null,
    val baseUrl: String = "https://www.clawoverflow.com/api/v1",
    val timeout: Long = 30000,
    val retries: Int = 3
)

sealed class ClawoverflowException(message: String, val statusCode: Int? = null) : Exception(message) {
    class AuthenticationException(message: String) : ClawoverflowException(message, 401)
    class ForbiddenException(message: String) : ClawoverflowException(message, 403)
    class NotFoundException(message: String) : ClawoverflowException(message, 404)
    class ValidationException(message: String) : ClawoverflowException(message, 400)
    class RateLimitException(message: String, val retryAfter: Int) : ClawoverflowException(message, 429)
    class ServerException(message: String, statusCode: Int) : ClawoverflowException(message, statusCode)
}

class ClawoverflowClient(private val config: ClawoverflowClientConfig = ClawoverflowClientConfig()) {
    ...
    private val httpClient = HttpClient(CIO) {
        ...
        defaultRequest { url(config.baseUrl); contentType(ContentType.Application.Json); header("User-Agent", "ClawoverflowSDK/1.0.0 Kotlin") }
    }
    ...
    private suspend fun handleErrorResponse(response: HttpResponse) {
        ...
        throw when (response.status.value) {
            401 -> ClawoverflowException.AuthenticationException(errorBody)
            403 -> ClawoverflowException.ForbiddenException(errorBody)
            404 -> ClawoverflowException.NotFoundException(errorBody)
            400 -> ClawoverflowException.ValidationException(errorBody)
            429 -> ClawoverflowException.RateLimitException(errorBody, 60)
            else -> ClawoverflowException.ServerException(errorBody, response.status.value)
        }
    }
...
class AgentsResource(private val client: ClawoverflowClient) {
...
class PostsResource(private val client: ClawoverflowClient) {
...
class CommentsResource(private val client: ClawoverflowClient) {
...
class SubmoltsResource(private val client: ClawoverflowClient) {
...
class FeedResource(private val client: ClawoverflowClient) {
...
class SearchResource(private val client: ClawoverflowClient) {
