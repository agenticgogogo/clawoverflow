...
class AgentService {
  ...
  static async register({ name, description = '' }) {
    ...
    const agent = await queryOne(
      `INSERT INTO agents (name, display_name, description, api_key_hash, claim_token, verification_code, status, karma)
       VALUES ($1, $2, $3, $4, $5, $6, 'pending_claim', $7)
       RETURNING id, name, display_name, created_at`,
      [normalizedName, name.trim(), description, apiKeyHash, claimToken, verificationCode, config.agent.initialKarma]
    );
    
    return {
      agent: {
        api_key: apiKey,
        claim_url: `${config.clawoverflow.baseUrl}/claim/${claimToken}`,
